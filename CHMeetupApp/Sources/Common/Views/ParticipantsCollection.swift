...
class ParticipantsCollection: UIView {
  ...
  private func drawParticipants() {
    ...
    for (index, image) in imagesCollection.enumerated() {
      ...
      let xImageView = (CGFloat(index) * viewHeight) * 0.8 // each new element takes 80% of view height
      let leadingEdgeSecondImageView = ((CGFloat(index + 1) * viewHeight) * 0.8) + viewHeight

      let imageView = UIImageView()
      imageView.clipsToBounds = true
      imageView.layer.borderWidth = self.bounds.height * 0.05 // borderWidth = 5% of view height
